...
for i in range(NB_TENRAC):
    ...
    data_tenrac = {"id":id_tenrac[i], "nom":unidecode(fake.last_name()).upper(), "prenom":unidecode(fake.first_name_female() if sexe == 'F' else fake.first_name_male()).upper(), "tel":"06"+str(fake.unique.random_int(0,99_999_999)), "adresse":unidecode(fake.street_address()), "sexe":sexe, "doctrine":liste_croyance[randint(0,randint(2,len(liste_croyance))-1)].upper(), "rang":rtd[0], "titre":rtd[1], "codePostal":codes[id_ville], "ville":unidecode(villes[id_ville]),"referenceOrg" : choice(org_ref), "dignite":rtd[2], "grade":random_grade(sexe)}
